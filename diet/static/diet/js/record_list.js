$(document).ready(function() {
    ...
    // input으로 date가 들어가고, date에 맞는 날짜에 selected_date class를 추가한다.
    ...
    function generateWeekCalendar(date) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            tbody += `<td id='${formattedDate}' class='text-center day_calendar_width'>${monthDay}</td>`;
    ...
    function changeWeek(days) {
        currentDate = new Date(selectedDate.setDate(selectedDate.getDate() + days));
